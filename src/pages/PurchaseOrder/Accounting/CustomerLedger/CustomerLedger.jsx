...
function CustomerLedger() {
    ...
    const columns = [
        ...
        { name: 'Balance', selector: (row) => row.groupName, sortable: true, width: '100px' },
        { name: '', selector: (row) => row.groupName, sortable: true },
    ];

    const data = dataTable.map((value, index) => ({ ...value, id: index + 1 }));

    return (
        <TitleLayout title={config.nameMap.itemLevel3.CUSTOMER_LEDGER.Visiblename}>
            {/* form */}
            <div className={cx('mb-2 flex flex-wrap justify-between')}>
                <div className={cx('flex flex-wrap gap-5')}>
                    ...
                    <Input label="Memo" paddingLabel boldLabel widthInput="300px" />
                </div>
