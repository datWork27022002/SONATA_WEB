...
function EnterNumberOfCustomerSetting() {
    const inputCommonProps = { radioInput: true, listOptions: listUseOrNotUse, labelClassName: cx('w-[200px]') };

    return (
        <TitleLayout title={config.nameMap.itemLevel3.ENTER_NUMBER_OF_CUSTOMER_SETTING.Visiblename}>
            <form className={cx('inline-block')}>
                {/* form */}
                <div className={cx('flex flex-wrap md:ml-8 md:gap-16')}>
                    <div className={cx('sm:w-[400px]')}>
                        <Input label="Enter the number of guests when ordering" {...inputCommonProps} />
                        <Input label="Customer analysis" {...inputCommonProps} />
                    </div>
                </div>
...
export default EnterNumberOfCustomerSetting;
