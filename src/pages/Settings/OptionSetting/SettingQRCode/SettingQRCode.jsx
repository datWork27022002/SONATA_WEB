...
function SettingQRCode() {
    return (
        <TitleLayout title={config.nameMap.itemLevel3.SETTING_QR_CODE.Visiblename}>
            <form className={cx('inline-block')}>
                {/* form */}
                <div className={cx('flex flex-wrap md:ml-8 md:gap-16')}>
                    <div className={cx('sm:w-[400px]')}>
                        <Input label="Type" dropDown />
                        <Input label="Url" />
                        <Input label="Start Date" type="date" />
                        <Input label="End Date" type="date" />
                        <Input label="Period type" radioInput listOptions={listUseOrNotUse} />
                    </div>
                </div>
