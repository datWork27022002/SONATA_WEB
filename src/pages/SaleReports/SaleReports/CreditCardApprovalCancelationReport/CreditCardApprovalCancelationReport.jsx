...
function CreditCardApprovalCancelationReport() {
    ...
    const columns = [
        ...
        { name: 'Receipt Number', selector: (row) => row.groupName, sortable: true, width: '150px' },
        ...
        { name: 'InstallMent month', selector: (row) => row.groupName, sortable: true, width: '120px' },
        { name: 'Approval Number', selector: (row) => row.groupName, sortable: true },
    ];
    const data = dataTable.map((value, index) => ({ ...value, id: index + 1 }));

    return (
        <TitleLayout title={config.nameMap.itemLevel3.CREDIT_CARD_APPROVAL_CANCELATION_REPORT.Visiblename}>
            {/* form */}
            ...
            <div className={cx('grid-cols-3 gap-x-5 lg:grid', 'max-lg:flex max-lg:flex-wrap')}>
                ...
                <div className={cx('col-start-3 mb-4 flex items-center justify-end gap-4')}>
...
export default CreditCardApprovalCancelationReport;
