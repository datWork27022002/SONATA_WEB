...
function EmployeeCommission() {
    ...
    const columns = [
        ...
        { name: 'No of Transaction', selector: (row) => row.groupName, sortable: true, width: '200px' },
        { name: 'Commission Amount', selector: (row) => row.groupName, sortable: true },
    ];

    const data = dataTable.map((value, index) => ({ ...value, id: index + 1 }));

    return (
        <TitleLayout title={config.nameMap.itemLevel3.EMPLOYEE_COMMISSION.Visiblename}>
            {/* form */}
